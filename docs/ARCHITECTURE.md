...
Worker payloads are treated as untrusted structured-clone data. Classic validation limits IDs to the SceneSpec ID envelope, coordinates to a conservative ±100 m transport envelope, physical/effective distances to 150/1,600 m, route points to 10, Portal/wall arrays to their 8/100 entity limits, low-pass to 20–20,000 Hz, early delay to 0–80 ms, and first-order taps to six. It also recomputes physical distance from the accepted source/Listener snapshot, requires route endpoints and route length to agree with that snapshot, and ties every reflection point, path length, and Portal-relative delay to its installed wall and the 343 m/s model. Hybrid validation uses the installed geometry as the authoritative patch/surface/material registry, bounds coordinates to ±100 m, direct distance to 250 m, delay to 0–2,000 ms, hit/reflection arrays to installed patch count, and requires unit arrival/propagation directions plus route/hit consistency. It independently recomputes direct distance, delay, direction, azimuth/elevation, hit locations, and each reflection's finite-patch point, path length, delay, excess delay, and arrival direction. These transport bounds are deliberately wider than valid authored rooms while still rejecting finite but hostile payloads.
