...
2. In **Concrete Partition**, close the Portal so the direct route is blocked. Confirm at least one purple two-bounce path has two reflection vertices and that placing an intervening Wall or basic-shape footprint removes any route/path that crosses it.
3. Load **Treated Room** while the source continues playing. Confirm that Mid/High RT60 are lower than Hard Room and listen for the less sustained / less bright room character. The curated sources are continuous loops, so a standalone impulse-tail audition is not exposed.
4. Compare the displayed Volume, Surface, and Pre-delay in the two fixed-size presets. Outer-room scaling is not an editor control; deterministic scale behavior is instead covered by room-acoustics unit tests.
5. Return to **Hard Room**, click a wall to select it, then drag one of that wall's revealed endpoint handles; move the source and listener continuously for 20 seconds during playback. Confirm there is no repeated click, burst, silence, or runaway feedback and that the UI remains responsive.
6. Confirm the UI says `Interactive acoustic approximation`; check that the displayed values are finite and that room values do not appear until the matching scene revision has computed.

Pass when the hard/treatment material contrast is visible and perceptually coherent, four first-order paths are visible for the canonical rectangular rooms, blocked Classic routes expose bounded two-bounce paths without crossing obstacles, all reported values are finite, and continuous edits remain stable. The Gate C model is an interactive acoustic approximation, not an architectural-acoustics measurement.
