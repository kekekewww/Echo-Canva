...
- Never embed or configure a shared provider credential in the deployment. A visitor may select only the allowlisted OpenAI or OpenRouter provider and keep a separate key for each in that tab's `sessionStorage`. Send only the active credential over HTTPS to a request-scoped same-origin server proxy; never persist, log, return, export, or place keys in project state.
